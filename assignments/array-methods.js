...
let fullName = [];
const namesort = runners.forEach(function(name){
...
let allCaps = [];
const names = runners.map(function(name){
...
let largeShirts = [];
const Lshirts = runners.filter((size) => { 
if (size.shirt_size === "L") { 
    return  largeShirts.push(`  ${size.first_name} ${size.last_name} due to an order issue you have to pick a size other than Large`)
}   
 });
...
// pull out each individual donation
// add each individual donation up
// push the sum back into ticketPriceTotal

let ticketPriceTotal = [];
const totalSum = [];
const donation = runners.forEach(function(donation){
totalSum.push(donation.donation)
return ticketPriceTotal = totalSum.reduce(function(a,b){
    return a + b;
})
});
...
// Problem 1
//The event organizer needs to send an email to all the runners, make an array with all the emails
 let emails = []
...
// Problem 2
// we decided to match each donatio and need to put each sum together.
 const map1 = runners.map(function(runner){
...
let companyPartnerships = []

const partners = runners.filter(function(partners){
   if (partners.company_name === "Wordtune"){
       companyPartnerships.push(partners.first_name)
   }
});
console.log(companyPartnerships);
